---
title: "M&A Analysis Example"
date: 2026-01-01
categories: case-studies
---

# I. Transaction Overview (Common)

- Acquirer:
- Target:
- Announcement Date:
- Deal Value:
- Consideration (Cash / Stock / Mixed):
- Transaction Structure (Merger, Asset Purchase, Tender, LBO, IPO, etc.):
- Status (Announced / Pending / Completed / Withdrawn):

# II. Deal Structure & Key Terms (Common)

- Legal structure (e.g., forward triangular merger, reverse triangular merger)
- Consideration mix (cash, stock, earn-out, other)
- Key closing conditions (shareholder approvals, regulatory, financing, MAE)
- Key covenants (no-shop, go-shop, ordinary course, non-compete)
- Deal protections (termination fee, reverse termination fee, matching rights, tail provisions)

# III. Strategic Rationale (Common)

## Acquirer Perspective
- Strategic logic (market entry, scale, scope, capabilities)
- Product / platform synergies
- Competitive positioning vs peers

## Target / Seller Perspective
- Business situation (growth, profitability, strategic challenges)
- Rationale for selling / going public / recapitalizing
- Alternatives considered (stay independent, other buyers, status quo)

# IV. Valuation & Pricing (Common)

- Standalone valuation methods used (DCF, trading comps, transaction comps)
- Implied valuation multiples (EV/Revenue, EV/EBITDA, P/E, etc.)
- Premiums:
  - Premium to 1‑day prior close:
  - Premium to 1‑month VWAP:
- Control premium vs peers and precedent transactions
- Brief commentary: Is the price aggressive, fair, or cheap?

# V. Synergies & Financial Impact (Common for Strategic M&A)

- Revenue synergies (cross-sell, up-sell, geographic/product expansion)
- Cost synergies (overhead, operations, procurement, tech)
- Timing: phasing of synergy realization
- Impact on:
  - EPS (accretion / dilution)
  - Margin profile
  - Return on invested capital (ROIC)

# VI. Market & Competitive Context (Common)

- Industry trends and growth drivers
- Competitive landscape at announcement
- Recent comparable deals or capital markets activity
- Macro backdrop (rates, risk sentiment, regulation)

# VII. Key Risks & Execution Challenges (Common)

- Integration risks (systems, culture, customers)
- Regulatory / antitrust risks
- Financial risks (leverage, refinancing, covenants)
- Deal-specific risks (key-person, technology, concentration)

# VIII. Process & Timeline (Common)

- Deal timeline (initial contact → LOI → signing → closing)
- Auction vs bilateral negotiation
- Role of advisors (banks, lawyers, other)
- Any notable process dynamics (competing bidders, leaks, activism)

# IX. Post-Deal Performance & Outcomes (Common)

- Short-term market reaction (acquirer and target stock on announcement)
- 6–12 month performance vs index / sector
- Whether stated strategic goals were achieved
- Any major post-deal news (impairments, divestitures, litigation)

# X. Analyst Take (Your View – Common)

- Was this a good deal for the acquirer? Why / why not?
- Was the price justified by synergies and strategic benefits?
- What would you have recommended as the bank (structure, price, timing)?
- Key lessons for future deals

---

## Optional Module A – LBO / Private Equity Add-On

### A1. Transaction Financing Structure

- Sources & Uses table (equity, debt tranches, fees)
- Debt structure (Term Loan, Notes, Mezzanine, RCF)
- Opening leverage (Net Debt / EBITDA, interest coverage)

### A2. Returns Analysis

- Entry vs exit assumptions (multiple, timing, deleveraging)
- Base-case IRR and MoM
- Sensitivity to exit multiple, leverage, and operating performance

### A3. Investment Thesis & Value Creation Plan

- Operational improvements (margin expansion, cost cuts)
- Growth initiatives (pricing, product, M&A roll-up)
- Governance / management changes

---

## Optional Module B – ECM (IPO / Follow-On) Add-On

### B1. Offering Details

- Offering type (Primary / Secondary / Mixed)
- Deal size (gross / net proceeds)
- Offer price vs range
- Implied market cap and free float
- Lead underwriters and syndicate

### B2. Sources & Uses of Proceeds

- Primary proceeds to company vs secondary to selling shareholders
- Estimated fees and expenses
- Net cash to company and specific use of proceeds

### B3. Dilution & Ownership

- Shares outstanding pre / post
- % dilution to existing shareholders
- Post-deal ownership by key holders (founders, PE, strategic)

### B4. Market Conditions

- Sector performance into the deal
- Volatility/risk indicators (e.g., VIX level) around pricing
- IPO / ECM window (hot / cold market; recent deals’ performance)

### B5. Post-Deal Trading

- First-day return
- Performance 1m / 3m after
- Aftermarket support, stabilization, and overhang considerations