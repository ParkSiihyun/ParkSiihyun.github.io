...
* Investment banks are looking for decisive candidates who can articulate and logically present their solutions and defend their decisions under scrutiny(정밀조사).
...
To complete the case study, you need to determine if the acquisition is feasible(실행 가능한). Second, what would be the structure and synergies of the deal if the buyer has access to capital? You need to use multiples and valuation metrics to determine the price range for the transaction.
...
* Ensure that key business concepts are well understood and that you can use the associated terminology(관련 용어) comfortably in a conversation.
* Learn about various valuation techniques, how to employ them, and how to interpret them. Learn how and when to utilize key valuation methodologies and the supporting calculations.
...
* Learn how to stand out from the crowd in your interview with the investment banker.
...
한 잡지 발행 회사가 주주가치를 최대화하기 위해서 

* 회사를 통째로 매각할지,

* 그냥 지금처럼 내부 성장(유기적 성장)을 계속할지,

* 아니면 작은 규모의 관련 회사들을 조금씩 인수(tuck-in acquisition)할지를 검토하고 있다. 

이 회사는 이런 옵션들을 두고 자문해 줄 투자은행을 고르는 중이고, 너희 은행에게 '우리 옵션들을 분석해서 보여주는 프레젠테이션을 제출해 달라'고 요청한 상황이다. 
...
* Make a decision-it’s usually best to say “sell” unless the industry is growing quickly (over 10% per year), the company is extremely undervalued, or there are acquisition targets that would boost revenue or profit by at least 20-30%.
  ...
  케이스에서 최종적으로 어떤 액션을 추천할 것인가에 대한 문제. 의사결정의 rule of thumb을 말하는 것이다. 보통은 매각이 최선이라고 말하는 게 좋지만 아래 경우들 중 하나라도 해당되면 예외일 수 있다.

  * 산업 성장률이 아주 빠른 편(연 10% 이상)인 경우
  * 회사가 시장에서 심하게 저평가되어 있는 경우
  * 인수를 하면 매출이나 이익이 최소 20~30% 이상 늘어날 수 있는 좋은 타겟들이 있는 경우

  산업 성장률이 연 10% 이상이라는 말은 앞으로 몇 년 동안 시장 자체가 크게 커져서 가만히 있어도 매출 성장 기회가 크다는 뜻이다. 이런 상황에서 회사를 매각하게 되면 앞으로 누릴 수 있는 이익을 다음 주인에게 넘겨주는 꼴이 된다. 기본적으로 지금 매각보다는 계속 보유해서 유기적 성장을 기대하는 전략이 낫다. 
